function contacts = opfeedback_archive(contacts, sensor, TB_params)
% stand-in for operator feedback. Uses archived feedback as user input.
%
...
new_img_ind = 1;
oper_ind = 1;
[confs, inds] = read_archive(TB_params.OPARCHIVE_PATH);
pause;
...
while 0 == 0
    if done == 1
        ...
        contacts(img_cnt).opfeedback.opconf = get_opconf(img_cnt);
...
% Find the opconf in the archive data for the input contact
    function opconf = get_opconf(cind)
      cind
...
% Read operator data from saved operator archive file
    function [confs, inds] = read_archive(filename)
